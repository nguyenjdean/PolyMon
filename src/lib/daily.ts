...
export function getDailyMeta(dateStr: string = getDailyDateString()): DailyMeta | null {
...
export function getDailySvg(mode: string, guessCount: number, dateStr: string = getDailyDateString()): string | null {
    const isLines = mode === 'lines';
    const isTriangles = mode === 'triangles';
